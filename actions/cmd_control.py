...
# MARK XXXV — macOS Terminal/Shell Control
...
MAC_COMMAND_MAP = [
    ...
    (["open ports", "listening ports", "netstat"],
     "lsof -i -P | grep LISTEN", False),
    (["wifi networks", "available wifi", "wireless networks"],
     "/System/Library/PrivateFrameworks/Apple80211.framework/Versions/A/Resources/airport -s", False),
    (["system info", "computer info", "hardware info", "mac info", "specs"],
     "system_profiler SPHardwareDataType", False),
    (["cpu usage", "processor usage"],
     "top -l 1 -s 0 | head -12", False),
    (["memory usage", "ram usage"],
     "vm_stat | head -10", False),
    (["macos version", "os version"],
     "sw_vers", False),
    (["installed apps", "installed software", "installed programs"],
    ...
    (["large files", "biggest files", "largest files"],
     'find ~ -type f -size +100M 2>/dev/null | head -10', False),
]
...
def _find_hardcoded(task: str) -> str | None:
    ...
    if any(ext in task_lower for ext in [".txt", ".log", ".md", ".csv"]):
